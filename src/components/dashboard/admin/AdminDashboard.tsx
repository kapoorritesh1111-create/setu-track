        ...
        <section className="setuSurfaceCard">
          <div className="setuSectionLead">
            <div>
              ...
              <div className="setuSectionHint">A safer shared signal layer for the highest-value blockers and follow-up actions.</div>
            </div>
            <button className="pill" onClick={() => router.push("/admin/notifications")}>Open notifications</button>
          </div>
          <div className="setuFocusList">
            {insights.opsNotifications.length ? insights.opsNotifications.slice(0, 5).map((item) => (
              <button className="setuFocusItem" key={item.id} onClick={() => router.push(item.href)}>
                <span>{item.title}</span>
                <strong>{severityLabel(item.severity)} • {item.metric}</strong>
              </button>
            )) : (
              <button className="setuFocusItem" onClick={() => router.push(`/reports/payroll?start=${encodeURIComponent(startDate)}&end=${encodeURIComponent(endDate)}`)}>
                <span>Payroll workspace</span>
                <strong>{insights.readinessLabel}</strong>
              </button>
            )}
          </div>
        </section>
