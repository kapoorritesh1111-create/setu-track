...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supa.rpc("payroll_close_blockers", {
    ...
    const totals = rows.reduce(
      (acc, r) => {
        acc.entries += Number(r.entries_count || 0);
    ...
    return NextResponse.json({ ok: true, period_start, period_end, blocked, totals, rows });
  } catch (e: any) {
